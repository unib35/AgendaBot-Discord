...
| 명령어 | 설명 | 특징 |
|--------|------|------|
| `/list` | 안건 목록 | 페이지네이션, 상태/담당자 필터링 |
| `/search` | 고급 검색 | 인터랙티브 필터, 키워드/상태/담당자/기간 검색 |
| `/stats` | 통계 대시보드 | 인터랙티브 버튼, 기간 비교, TOP 기여자 |

### AI 요약
| 명령어 | 설명 | 특징 |
|--------|------|------|
| `/summary` | AI 요약 | Gemini 2.0 Flash 모델, 인터랙티브 기간 선택, 상태 필터, 채널 게시 |
...
| `/setupkey` | API 키 설정 | 모달로 안전한 Gemini API 키 입력 (암호화 저장) |
| `/checksetup` | 설정 확인 | 현재 서버 설정 상태 확인 |
| `/clear` | 데이터 정리 | 완료/취소된 안건 정리, 빠른 삭제 모드 지원 |
| `/reminder` | 리마인더 관리 | 인터랙티브 대시보드, 일괄 스누즈/삭제 |
