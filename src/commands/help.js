...
2️⃣ **회의 시간 설정** - 빠른 선택 버튼 또는 사용자 지정 날짜
3️⃣ **리마인더 설정** - 자동 알림 설정 (기본/간단/전체/커스텀)
4️⃣ **상세정보 입력** - 모달창에서 제목, 배경, 목표 등을 입력
5️⃣ **체크리스트 추가** - 선택적으로 체크리스트 항목을 추가`
            },
            {
                name: '💡 입력 필드 설명',
...
• 리마인더 자동 예약 (1일 전, 1시간 전, 정시)
• 반복 회의 설정 가능 (주간/격주/월간)
• 체크리스트 패널 자동 생성`
            },
            {
                name: '📌 사용 예시',
...
• 완료된 안건도 수정 가능합니다`
    ...
    clear: {
        title: '🗑️ /clear - 안건 정리',
        description: '완료되거나 취소된 안건들을 정리하는 관리자 전용 명령어입니다.',
        fields: [
            {
                name: '📋 사용 방법',
                value: `\`/clear scope:quick\` - 완료+취소 안건 빠른 정리
\`/clear scope:fast\` - 7일+ 오래된 완료/취소 전체
\`/clear scope:old days:30\` - 30일 이상 오래된 안건
\`/clear scope:reminders\` - 리마인더만 삭제
\`/clear scope:select\` - 선택하여 삭제`
            },
            {
                name: '⚡ 빠른 삭제 모드',
                value: `\`skip_confirm:true\` 옵션으로 확인 없이 즉시 삭제
위험하지만 대량 정리 시 유용`
            },
            {
                name: '🗜️ 삭제 범위',
...
• 삭제 후 복구 불가능`
    ...
    reminder: {
        title: '🔔 /reminder - 리마인더 관리',
        description: '안건 리마인더를 관리하는 명령어입니다.',
        fields: [
            {
                name: '📋 주요 기능',
                value: `• **대시보드** - 모든 리마인더 한눈에 보기
• **일괄 스누즈** - 여러 리마인더 한번에 연기
• **필터링** - 오늘/내일/7일/스누즈됨/전송완료
• **빠른 조정** - 30분/1시간/2시간/내일/다음 영업일`
            },
            {
                name: '🎮 사용 방법',
                value: `1️⃣ \`/reminder\` 명령어 입력
2️⃣ 필터 버튼으로 리마인더 탐색
3️⃣ 체크박스로 여러 개 선택
4️⃣ 작업 버튼으로 일괄 처리`
            },
            {
                name: '🔄 리마인더 정책',
                value: `**기본** - 1일 전(09:00), 1시간 전, 정시
**간단** - 1시간 전만
**전체** - 1일 전, 3시간 전, 1시간 전, 정시
**커스텀** - 직접 시간 선택
**끄기** - 리마인더 없음`
            },
            {
                name: '💁 팁',
                value: `• 스누즈된 리마인더는 필터로 찾을 수 있음
• "다음 영업일" 옵션은 주말을 건너뛰
• 일괄 삭제로 오래된 리마인더 정리`
            }
        ]
        ...
        .addStringOption(option =>
            option.setName('command')
                ...
                .addChoices(
                    ...
                    { name: '🗑️ clear - 안건 정리', value: 'clear' },
                    { name: '🔔 reminder - 리마인더', value: 'reminder' },
